...
# ============================================================================
# Vortex Authorization Service - Registration Flow Test Runner
# ============================================================================
# Script for running registration and account creation tests with various options
# 
# Usage: ./run-auth-registration-tests.sh [OPTIONS]
# 
# Options:
...
#
# Examples:
#   ./run-auth-registration-tests.sh --unit --verbose
#   ./run-auth-registration-tests.sh --integration --coverage
#   ./run-auth-registration-tests.sh --all --profile test
#   ./run-auth-registration-tests.sh --filter "register" --quick
# ============================================================================
...
REPORT_DIR="$BASE_DIR/test-reports/auth-registration"
COVERAGE_DIR="$BASE_DIR/coverage-reports/auth-registration"
...
run_unit_tests() {
    ...
    local test_cmd="$MVN_CMD test $MVN_ARGS -Dtest=SimpleAuthServiceTest,SimplePasswordServiceTest"
...
run_integration_tests() {
    ...
    local test_cmd="$MVN_CMD test $MVN_ARGS -Dtest=ValidationTest"
    ...
    if $test_cmd; then
        print_results 0 "Integration Tests"
        return 0
    else
...
run_all_tests() {
    ...
    local test_cmd="$MVN_CMD test $MVN_ARGS -Dtest=SimpleAuthServiceTest,SimplePasswordServiceTest,ValidationTest"
    ...
    if $test_cmd; then
        print_results 0 "All Tests"
        return 0
    else
        print_results 1 "All Tests"
        return 1
    fi
}
...
# Main execution
print_header "Vortex Auth Registration Test Runner"
